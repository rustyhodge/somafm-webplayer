...
angular.module("somafmPlayerApp").factory("PlayerService", [
  "$q",
  "$rootScope",
  "StationService",
  "StorageService",
  function($q, $rootScope, StationService, StorageService) {
    ...
    var loadStreams = function(station) {
      angular.forEach(station.streamUrls.reverse(), function(url) {
        ...
        source.type = self.audioNode.canPlayType("audio/mpeg;")
          ? "audio/mpeg"
    ...
    var play = function(station) {
      return $q(function(resolve, reject) {
        ...
        if (station.streamUrls != null) {
          ...
          StationService.getStationPls(station._id).then(function(data) {
    ...
    var isPlaying = function(station) {
      return $rootScope.playingStation && $rootScope.playingStation === station
        ? station.playing
        : null;
    };
    ...
    var toggleMute = function() {
      ...
      setVolume(self.audioNode.muted ? 0 : self.lastVolume);
    };
